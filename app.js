...
.get(function(req,res){
  Article.findOne({title:req.params.articleTitle},function(err,foundArticles){
    if(foundArticles){
      ...
      res.send("no article matching with that title was found");
    }
  })
...
.delete(function(req,res){
  Article.deleteOne({title:req.params.article},{title:req.body.title,content:req.body.content},function(err){
    if(!err){
      res.send("successfully deleted");
    }
    else{
